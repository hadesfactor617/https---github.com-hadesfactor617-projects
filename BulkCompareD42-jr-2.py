...
def loadMachinesD42(doqlOutputFilePath):
    """Returns lists of Machines from DOQL Output JSON file"""

    with open(doqlOutputFilePath, 'r') as file:
        data = file.read()
    return json.loads(data)


# Loads excel file in the form of api bulk insert from D42
...
# Iterates over list pulled from D42 and load_excel and outputs dup serials
def iter_data(d42MachineData, dataFromExcel):
    dup_serials = list()
    notInD42 = list()

    for xlsSerial, m in dataFromExcel.items():
        inD42 = 0
        for v in d42MachineData:
            if len(v['serial_no']) == 0:
                continue
            s = v['serial_no'].upper()
            r = v['Rack Name']
            if s == xlsSerial:
                dup_serials.append(v)
                inD42 = 1
        if inD42 == 0:
            notInD42.append(m)
    return dup_serials, notInD42
